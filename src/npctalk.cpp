        ...
        std::string_view member, const std::string_view )
{
    ...
    return [input_var, dov_x_adjust, dov_y_adjust, dov_z_adjust, z_override,
        ...
        tripoint_abs_ms target_pos = get_tripoint_ms_from_var( input_var, d );
        ...
        const std::string_view, bool is_npc )
{
    ...
    return [dov, transform, target_var, dov_time_in_future, key, is_npc]( dialogue & d ) {
        ...
        if( target_var.has_value() ) {
            target_pos = get_tripoint_ms_from_var( *target_var, d );
        } else {
                                     ...
                                     const std::string_view, bool is_npc )
{
    ...
    return [target_var, dov_power, dov_distance_factor, dov_max_noise, fire, dov_shrapnel_casing_mass,
        ...
        if( target_var.has_value() ) {
            tripoint_abs_ms abs_ms = get_tripoint_ms_from_var( *target_var, d );
                                      ...
                                      const std::string_view, bool is_npc )
{
    ...
    return [type, target_var, message, range, z_level, center_var, is_npc]( dialogue & d ) {
        ...
        if( center_var.has_value() ) {
            tripoint_abs_ms abs_ms = get_tripoint_ms_from_var( *center_var, d );
        ...
        const std::string_view )
{
    ...
    return [center_var, relative_var, output_var]( dialogue & d ) {
        tripoint_abs_ms const center = get_tripoint_ms_from_var( center_var, d );
        tripoint_abs_ms const relative = get_tripoint_ms_from_var( relative_var, d );
        ...
        const std::string_view )
{
    ...
    return [transform, first, second]( dialogue const & d ) {
        tripoint_abs_ms const t_first = get_tripoint_ms_from_var( first, d );
        tripoint_abs_ms const t_second = get_tripoint_ms_from_var( second, d );
        ...
        const std::string_view )
{
    ...
    return [target_params, update_ids, target_var, dov_time_in_future, key]( dialogue & d ) {
        ...
        if( target_var.has_value() ) {
            const tripoint_abs_ms abs_ms( get_tripoint_ms_from_var( *target_var, d ) );
                                     ...
                                     const std::string_view, bool is_npc )
{
    ...
    return [target_var, unique_id, is_npc]( dialogue const & d ) {
        ...
        if( guy ) {
            ...
            tripoint_abs_ms target_location = get_tripoint_ms_from_var( cur_var, d );
        ...
        if( !caster ) {
            ...
            if( targeted ) {
                ...
                const tripoint_bub_ms target_pos = loc_var ?
                                                   get_map().get_bub( get_tripoint_ms_from_var( *loc_var, d ) ) : caster->pos_bub();
    ...
    return [is_npc, method, target_var]( dialogue & d ) {
        ...
        if( guy ) {
            ...
            if( it && it->get_item() ) {
                ...
                if( target_var.has_value() ) {
                    tripoint_abs_ms target_pos = get_tripoint_ms_from_var( *target_var, d );
                    if( get_map().inbounds( target_pos ) ) {
                                        ...
                                        const std::string_view src, bool is_npc )
{
    ...
    return [is_npc, eocs, target_var, cond, range, store_coordinates_in,
        ...
        tripoint_abs_ms pos;
        if( target_var.has_value() ) {
            pos = get_tripoint_ms_from_var( *target_var, d );
        } else {
                                     ...
                                     const std::string_view, bool is_npc )
{
    ...
    return [force, stun, dam_mult, target_var, direction_var, is_npc]( dialogue & d ) {
        ...
        if( direction_var.has_value() ) {
            tripoint_abs_ms abs_ms = get_tripoint_ms_from_var( *direction_var, d );
    ...
    return [is_npc, dmg_type, dmg_amount, bodypart, arpen, arpen_mult, dmg_mult, dbl_min_hit,
        ...
        if( d.actor( is_npc )->get_character() ) {
            if( bp_str == "RANDOM" ) {
                ...
                bp = guy.select_body_part( &guy, min_hit, max_hit, can_attack_high, hit_roll );
            }
        }
        ...
        const std::string_view src, bool is_npc )
{
    ...
    return [monster_id, dov_target_range, dov_hallucination_count, dov_real_count, dov_min_radius,
        ...
        if( target_var.has_value() ) {
            target_pos = here.get_bub( get_tripoint_ms_from_var( *target_var, d ) );
        }
                                      ...
                                      const std::string_view )
{
    ...
    return [ radius, target_var ]( dialogue & d ) {
        tripoint_abs_omt omt = project_to<coords::omt>( get_tripoint_ms_from_var( target_var, d ) );
                                        ...
                                        const std::string_view )
{
    ...
    return [ radius, from, to, road_only ]( dialogue & d ) {
        tripoint_abs_ms from_pos = get_tripoint_ms_from_var( from, d );
        tripoint_abs_omt omt_from = project_to<coords::omt>( from_pos );
        tripoint_abs_ms to_pos = get_tripoint_ms_from_var( to, d );
                                        ...
                                        const std::string_view )
{
    ...
    return [var, type, var_name, known]( dialogue & d ) {
        tripoint_abs_ms loc_ms = get_tripoint_ms_from_var( var, d );
