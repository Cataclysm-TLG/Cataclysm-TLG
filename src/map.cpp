...
bash_params map::bash( const tripoint_bub_ms &p, const int str,
                       bool silent, bool destroy, bool bash_floor, bool fire,
                       const vehicle *bashing_vehicle, bool crystalline_only )
{
    bash_params bsh{
        str, silent, destroy, bash_floor, fire, static_cast<float>( rng_float( 0, 1.0f ) ), false, false, false, false
    };
    if( !inbounds( p ) ) {
        return bsh;
    }
                                ...
                                int eye_level ) const
{
    ...
    [this, eye_level, &visible, &to, &last_point]( const tripoint_bub_ms & new_point ) {
        ...
        // Z-level transition (vertical line)
            ...
            const int max_z = std::max( new_point.z(), last_point.z() );
            tripoint_bub_ms floor1 = { new_point.xy(), last_point.z() };
            tripoint_bub_ms floor2 = { last_point.xy(), new_point.z() };
...
std::vector<tripoint_bub_ms> map::reachable_flood_steps(
    const tripoint_bub_ms &f, int range,
    ...
    for( int z = 0; z < z_dim; ++z ) {
        for( int y = 0; y < grid_dim; ++y ) {
            for( int x = 0; x < grid_dim; ++x ) {
                int ndx = grid_index( x, y, z );
                if( o_grid[ndx] ) {
                    reachable_pts.push_back( f - origin_offset + tripoint( x, y, z ) );
                }
            }
        }
    }
                      ...
                      const int cost_min, const int cost_max ) const
{
    ...
    [this, &is_clear, cost_min, cost_max, t, &last_point]( const tripoint_bub_ms & new_point ) {
        ...
        if( new_point.z() == last_point.z() ) {
            ...
            const tripoint_bub_ms &lower = going_up ? last_point : new_point;
            const tripoint_bub_ms &upper = going_up ? new_point : last_point;
...
bool map::try_fall( const tripoint_bub_ms &p, Creature *c )
{
    ...
    if( you->has_flag( json_flag_WALL_CLING ) &&  this->is_clingable_wall_adjacent( p ) ) {
