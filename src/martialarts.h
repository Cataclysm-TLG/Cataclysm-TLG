...
struct attack_vector {
    ...
    // Bodypart cannot be covered by anything that doesn't have ALLOWS_NATURAL_ATTACKS flag
    bool natural_attack = true;
...
class ma_technique
{
    public:
        ...
        bool fallback = false;    // tech will only be used if no others are available
        bool basic = false;    // tech does not prevent fallbacks
