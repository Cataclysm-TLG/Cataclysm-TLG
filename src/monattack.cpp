...
void mattack::flame( monster *z, Creature *target )
{
    ...
    if( target != &player_character ) {
        ...
        std::vector<tripoint_bub_ms> traj = here.find_clear_path( z->pos_bub(), target->pos_bub() );

        for( tripoint_bub_ms &i : traj ) {
...
bool mattack::flesh_tendril( monster *z )
{
    ...
    if( target == nullptr || !z->sees( here, *target ) ) {
        if( one_in( 70 ) ) {
            ...
            z->mod_moves( -to_moves<int>( 2_seconds ) );
            sounds::sound( z->pos_bub(), MAX_VIEW_DISTANCE, sounds::sound_t::alert, _( "a deafening roar!" ), false, "shout",
                           "roar" );
        }
        return false;
    }
    ...
    if( ( distance_to_target == 2 || distance_to_target == 3 ) && one_in( 4 ) ) {
        ...
        if( pulled && one_in( 4 ) ) {
            sounds::sound( z->pos_bub(), MAX_VIEW_DISTANCE, sounds::sound_t::alarm, _( "a deafening roar!" ), false, "shout",
                           "roar" );
        }
        return pulled;
    }
